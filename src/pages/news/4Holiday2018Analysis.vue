    ...
    <div class="box mt-0 pl-2 pr-2">
      <v-layout class="mainContainer boxContent mt-5 pl-4 pr-4" row nowrap>
        <v-layout class="text-xs-left pb-3" column wrap>
          ...
          <span class="title mt-4">Highest winrate between Decks with at least 10 games</span>
          <span class="body-1 mt-3">
            Although with just 11 games, a very nice surprise here is that a 4-Colors deck was the deck with highest winrate.
          ...
          <v-layout row class="text-xs-center">
            <Deck class="mt-2 mb-2 m-auto" :cards="deckWithHighestWinRate.cards" :name="deckWithHighestWinRate.name"
              :details="`${deckWithHighestWinRate.games} games (${deckWithHighestWinRate.winrate}% winrate)`" :link="true"/>
          </v-layout>
            
          <span class="title mt-4">Decks with best consistense by winrate</span>
          <span class="body-1 mt-3">
            The follow list of deck, show decks with at least 20 games sorted by winrate.
          </span>
          <v-layout class="mt-2 text-xs-center" row wrap>
            <v-layout column class="deck mt-3 ml-2" v-for="deck in decksHighestWinRateAndConsistence" :key="deck.id">
...
export default {
  ...
  data () {
    return {
      ...
      decksByGuildsData: {},
      decksByGuildsLabels: {},
      deckWithHighestWinRate: HighestWinRate[0],
      decksHighestWinRateAndConsistence: HighestWinRate.filter(deck => deck.games >= 20).slice(0, 8),
